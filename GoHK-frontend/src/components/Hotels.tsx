...
import { type accom } from '../Types/accomodation';
...
const Hotels: FC = () => {
  const hotelsAndHostels: accom[] = [
    ...
    },
    {
      ...
      name: "Panda Hotel",
      address: "Tsuen Wan",
      description: "Modern family-friendly hotel in Tsuen Wan, Hong Kong, offering spacious rooms, convenient transport links, dining options, and welcoming hospitality comfort.",
      rating: 4.15,
      price: 500,
      pros: [
          ...
          <div> {/* list of hotels and hostels */}
            {hotelsAndHostels.map((hot, index) => (
              <div key={index} className='flex flex-col min-[1300px]:flex-row mt-5 first:mt-0 border rounded-lg items-stretch border-black/10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1),0_4px_6px_-1px_rgba(0,0,0,0.1)]'>
                <img src={hot.image} alt="" className="w-full min-[1300px]:w-100 min-[1300px]:h-50 object-cover rounded-l-lg shrink-0" />
